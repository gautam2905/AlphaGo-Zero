...
def board_augment(board, policy, board_size):
    """Apply random augmentation to board state and policy.
    
    Args:
        board: numpy array of shape (C, H, W) - board state
        policy: numpy array of shape (action_size,) - action probabilities
        board_size: size of the board
    
    Returns:
        augmented_board: transformed board state
        augmented_policy: transformed policy probabilities
    """
    import numpy as np
    
    # Convert to torch tensors
    board_tensor = torch.tensor(board, dtype=torch.float32).unsqueeze(0)  # Add batch dimension
    policy_tensor = torch.tensor(policy, dtype=torch.float32).unsqueeze(0)  # Add batch dimension
    values_tensor = torch.tensor([0.0], dtype=torch.float32)  # Dummy value
    
    # Apply random transformation
    aug_board, aug_policy, _ = apply_random_transformation(board_tensor, policy_tensor, values_tensor)
    
    # Convert back to numpy and remove batch dimension
    augmented_board = aug_board.squeeze(0).numpy()
    augmented_policy = aug_policy.squeeze(0).numpy()
